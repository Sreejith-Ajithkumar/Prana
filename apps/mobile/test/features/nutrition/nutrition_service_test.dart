...
void main() {
  group('NutritionService', () {
    ...
    test('calculates complete nutrition targets for weight loss', () {
      ...
      expect(targets.bmi, closeTo(24.691, 0.001));

      expect(targets.bmiCategory, BmiCategory.healthy);

      expect(targets.bmr, closeTo(1780, 0.001));

      expect(targets.tdee, closeTo(2759, 0.001));

      expect(targets.calories, closeTo(2259, 0.001));

      expect(targets.protein, closeTo(160, 0.001));

      expect(targets.fat, closeTo(75.3, 0.01));

      expect(targets.carbohydrates, closeTo(235.325, 0.01));

      expect(targets.waterLitres, closeTo(2.64, 0.001));
    });
    ...
    test('calculates maintenance targets for female profile', () {
      ...
      expect(targets.bmi, closeTo(22.039, 0.001));

      expect(targets.bmiCategory, BmiCategory.healthy);

      expect(targets.bmr, closeTo(1320.25, 0.001));

      expect(targets.tdee, closeTo(1584.3, 0.001));

      expect(targets.calories, closeTo(1584.3, 0.001));

      expect(targets.protein, closeTo(84, 0.001));

      expect(targets.fat, closeTo(52.81, 0.01));

      expect(targets.carbohydrates, closeTo(193.25, 0.01));
    ...
    test('throws when biological sex is unspecified', () {
      ...
      expect(() => service.calculate(profile), throwsUnsupportedError);
    });
  });
}
